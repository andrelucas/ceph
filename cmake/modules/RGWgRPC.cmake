...
function(buf_grpc_expand var_proto var_header var_source proto_dir proto_list output_dir)
    ...
    foreach(input ${proto_list})
        ...
        list(APPEND proto ${proto_dir}/${input}.proto)
        list(APPEND hdr ${output_dir}/${input}.pb.h)
        list(APPEND hdr ${output_dir}/${input}.grpc.pb.h)
        list(APPEND src ${output_dir}/${input}.pb.cc)
        list(APPEND src ${output_dir}/${input}.grpc.pb.cc)
        list(APPEND src ${output_dir}/${input}_pb2.py)
        list(APPEND src ${output_dir}/${input}_pb2_grpc.py)
    endforeach()

    # Export var_header and var_source (can't use return(PROPAGATE), we can't
    # rely on having CMake >= 3.25).
    set(${var_proto} "${proto}" PARENT_SCOPE)
    set(${var_header} "${hdr}" PARENT_SCOPE)
    set(${var_source} "${src}" PARENT_SCOPE)

    message(VERBOSE "grpc_expand: ${var_proto}: ${proto}")
    message(VERBOSE "grpc_expand: ${var_header}: ${hdr}")
    message(VERBOSE "grpc_expand: ${var_source}: ${src}")
