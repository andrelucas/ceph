 ...
 * attempting authentication that might have an authorization element. Doing
 * work on an unauthenticated request is in general a bad idea and risks
 * introducing potential security problems. For example, we've done nothing to
 * validate the bucket and object key names yet, though they will at least
 * have been URL decoded.
 *
 * @param dpp DoutPrefixProvider.
 * @param s The request (const).
 */
AuthorizationParameters::AuthorizationParameters(const DoutPrefixProvider* dpp, const req_state* s) noexcept
{
  valid_ = false;

  if (s == nullptr) {
    ldpp_dout(dpp, 0) << "AuthorizationParameters: invalid request pointer" << dendl;
    return;
  }

  // Method should be set in the request.
  if (!s->info.method || *(s->info.method) == 0) {
    ldpp_dout(dpp, 0) << "AuthorizationParameters: Invalid request method" << dendl;
    return;
  }
  ...
  ldpp_dout(dpp, 20) << "AuthorizationParameters: req_name='" << req_name << "'" << dendl;
  ...
  if (*req_name != '/') {
    ldpp_dout(dpp, 0) << "AuthorizationParameters: Invalid relative_uri string" << dendl;
    return;
  }
  req_name++;

  // An empty request portion isn't that useful to authorization, but is valid
  // in general use - it's generated by e.g. 's3cmd ls'. We'll return an
  // object that has very little information, but _is_ valid.
  //
  if (*req_name == 0) {
    ldpp_dout(dpp, 0) << "AuthorizationParameters: No query string information available" << dendl;
    valid_ = true;
    return;
  }
    ...
    optional_yield y)
{
  ...
  if (!(authorization_param->valid())) {
    ...
    ldpp_dout(dpp, 0) << "Handoff: AuthorizationParameters not available" << dendl;
